...
const Home = () => {
    ...
    useEffect(() => {
        ...
        // Fetch History only if logged in
        if (userEmail) {
            const loadHistory = async () => {
                const history = await userService.getUserBoardHistory(userEmail);
                                        ...
                                        <div className="max-h-32 overflow-y-auto bg-slate-800/50 rounded-lg border border-gray-700/50 custom-scrollbar">
                                            {recentBoards.map((board) => (
                                                <button
                                                    ...
                                                    className="w-full text-left px-3 py-2 text-sm text-gray-300 hover:bg-white/10 hover:text-white transition-colors flex justify-between items-center border-b border-gray-700/30 last:border-0"
                                                >
