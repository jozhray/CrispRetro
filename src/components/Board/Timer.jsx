...
const Timer = ({
    ...
    const handleStartPause = () => {
        ...
        const currentDuration = timer.duration || selectedSeconds || 180;

        if (!timer.isRunning && !timer.timeLeft) {
            onUpdateTimer({ ...timer, isRunning: true, timeLeft: currentDuration, duration: currentDuration });
        } else {
            onUpdateTimer({ ...timer, isRunning: !timer.isRunning, duration: currentDuration });
        }
    };
    ...
    const handleSelectTime = (seconds) => {
        ...
        onUpdateTimer({ ...timer, timeLeft: seconds, duration: seconds, isRunning: false });
        setShowPicker(false);
    };
    ...
    const totalTime = timer.duration || selectedSeconds || 180;
    const progress = totalTime > 0 ? ((totalTime - displayTime) / totalTime) * 100 : 0;

    return (
        <>
            <div className="relative flex items-center gap-2 bg-gradient-to-r from-slate-800 to-slate-900 px-2.5 rounded-lg shadow-lg h-8">
                ...
                <div className="relative">
                    <svg className="w-5 h-5 transform -rotate-90">
                        ...
                        <circle
                            ...
                            fill="none"
                            strokeDasharray={51}
                            strokeDashoffset={51 - (51 * progress) / 100}
                            strokeLinecap="round"
                        />
                    </svg>
                ...
                <div className="flex flex-col items-start min-w-[42px]">
                    <span className={`text-base font-bold font-mono leading-none tracking-wider ${timer.timeLeft <= 10 && timer.isRunning ? 'text-red-400 animate-pulse' : 'text-white'}`}>
                ...
                {isAdmin && (
                    <div className="flex items-center gap-1">
                        <button
                            ...
                            className={`p-1 rounded-md transition-all ${timer.isRunning
                                ? 'bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30'
                                : 'bg-green-500/20 text-green-400 hover:bg-green-500/30'
                                }`}
                            title={timer.isRunning ? "Pause" : "Start"}
                        >
                            {timer.isRunning ? <Pause size={12} fill="currentColor" /> : <Play size={12} fill="currentColor" />}
                        </button>
